...
const tree=process.argv.includes('--trees');
const derived=path.join(root,'output/integration/'+(tree?'tree_trial_v002':'nature_trial_v001'));
const dest=path.join(repo,tree?'Data/Provenance/TreeTrial/v002':'Data/Provenance/NatureTrial/v001');
const prefix=tree?'data://provenance/treetrial/v002/':'data://provenance/naturetrial/v001/';
...
function texture(name,bytes,group){const key=group+'/'+crypto.createHash('sha256').update(bytes).digest('hex');if(tree&&sharedTextures.has(key))return sharedTextures.get(key);binary.push({name:name+'.png',bytes});const result=resource(name+'.texture',`<Type TypeID="EE::Render::TextureResourceDescriptor" Version="0">\n  <Property ID="m_sourcePaths">\n    <Property Index="0" Value="${prefix+name}.png" />\n  </Property>\n  <Property ID="m_textureGroup" Value="data://render/texturegroups/${group}.txtg" />\n</Type>\n`);sharedTextures.set(key,result);return result;}
for(const entry of report.filter(e=>!process.env.TRIAL_ASSET||e.id===process.env.TRIAL_ASSET)){
 ...
 const mats=j.materials.map((m,i)=>{
  ...
  const flags=[...(m.doubleSided?['TwoSided']:[]),...(tree&&m.alphaMode==='MASK'?['AlphaTest']:[])].join('|');
  const props=[prop('m_shaderFlags',flags),prop('m_albedoTexture',albedo),prop('m_pbrTexture',pbr)];
  if(tree&&m.alphaMode==='MASK')props.push(prop('m_alphaCutoff',m.alphaCutoff??.5));
...
// Optional block-aligned runtime copies; original embedded image remains in GLB.
if(tree)for(const b of binary){const aligned=path.join(derived,'aligned_textures',b.name);if(fs.existsSync(aligned)){b.original=b.bytes;b.bytes=fs.readFileSync(aligned);}}
if(tree)for(const b of binary){const aligned=path.join(derived,'aligned_textures_pow2',b.name);if(fs.existsSync(aligned)){b.previous=b.bytes;b.bytes=fs.readFileSync(aligned);}}
if(process.argv.includes('--install-binaries')){
 fs.mkdirSync(dest,{recursive:true});
 for(const b of binary){const target=path.join(dest,b.name);if(fs.existsSync(target)){const old=fs.readFileSync(target);if(!old.equals(b.bytes)&&!(b.original&&old.equals(b.original))&&!(b.previous&&old.equals(b.previous)))throw Error('Refusing to overwrite different file '+target);}fs.writeFileSync(target,b.bytes);}
